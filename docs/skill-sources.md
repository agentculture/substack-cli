...
substack-cli vendors its `.claude/skills/` from **guildmaster** — the
...
#   - consumer-identifying prose: `guildmaster` → `substack-cli` (NOT
...
If a re-sync would lose a substack-cli adaptation, lift the change
...
#  `which substack-cli vendors from guildmaster`).

...
`substack-cli#25` stays open for the existing fleet.
...
- **Worktree location convention** in `CLAUDE.md` — every worktree you create
  ...
  `../.worktrees.substack-cli/<name>/`, one
...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`substack-cli vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/substack-cli/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `substack_cli/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=substack_cli` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/substack-cli/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/substack-cli/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: substack-cli`,

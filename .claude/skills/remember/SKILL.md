...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope substack-cli
...
scope:** the default personal scope is **private** (`--scope substack-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `substack-cli`). Private records are served only to a recall in the same
